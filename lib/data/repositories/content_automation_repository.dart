...
class ContentAutomationRepository {
  ...
  Future<List<ContentSource>> fetchSources() async {
    final response = await _client
        ...
        .select('id,name,source_url,feed_url,source_type,trust_level,enabled,last_checked_at')
  ...
  Future<AutomationSettings> fetchSettings() async {
    final response = await _client
        ...
        .select('enabled,max_articles_per_run,max_drafts_per_day,monthly_draft_cap,manual_cooldown_minutes,max_pending_drafts,max_pending_questions,draft_archive_days,rejected_delete_days,archived_delete_days,last_manual_run_at')
        .eq('id', 1)
        .maybeSingle();
  ...
  Future<List<GeneratedContentDraft>> fetchDrafts() async {
    final response = await _client
        ...
        .select('id,title,summary,topic_id,target_level,source_name,source_url,source_published_at,status,draft_payload,created_at')
        .eq('status', 'draft')
  ...
  Future<String> runVerificationDraftsNow() async {
    ...
    if (response.status < 200 || response.status >= 300) {
      ...
      throw StateError(
        detail == null || detail.trim().isEmpty
            ? 'Verification draft generation could not be started.'
            : detail,
      );
    }
    if (data is Map) {
    ...
    return 'Verification draft check completed.';
  }
