...
class DictionaryController extends ChangeNotifier {
  ...
  Future<void> lookup(String word, {bool forceOnline = false}) async {
    ...
    // Dictionary definitions are stable. Reuse an already saved definition
    // before spending another external API request; Retry explicitly bypasses
    // this cache when the learner wants a fresh online lookup.
    if (!forceOnline) {
      final saved = await _readSavedDefinition(_lastWord);
      if (saved != null) {
        _entry = saved;
        _hasSearched = true;
  ...
  Future<DictionaryEntry?> _readSavedDefinition(String word) async {
    try {
      await _storage.init();
      final cache = _readCacheMap();
      final raw = cache[word.toLowerCase()];
      if (raw is Map) {
        return DictionaryEntry.fromMap(Map<String, dynamic>.from(raw));
      }
    } catch (_) {
      // Cache is an optimization only; online lookup remains authoritative.
    }
    return null;
  }
