...
class AdaptiveLearningController extends ChangeNotifier {
  ...
  Future<void> synchronizeExistingProgress({
    ...
    _pendingExistingProgressQuizzes = List<Quiz>.from(quizzes);
    _pendingExistingProgressScores = Map<String, int>.from(bestScores);
  ...
  Future<void> _synchronizeExistingProgressOnce({
    ...
    if (repository != null) {
      try {
        final existing = await repository.fetchQuestionAttempts(
          userId,
          countedOnly: true,
        );
        if (_activeUserId != userId) return;
        existingItemTopics.addAll(existing.map((item) => item.itemTopicKey));
        _rememberRemoteAttemptState(existing);
      } catch (_) {
        // The local path is still idempotent. A later refresh will let the
        // server-authoritative attempt RPC reconcile any queued legacy import.
      }
    }
    ...
    for (final quiz in completed) {
      ...
      if (changed) existingItemTopics.add(itemTopicKey);
    }

    if (repository != null && _activeUserId == userId) {
      await _loadRemoteState(userId, preserveLocalNewerState: true);
    }
  }
  ...
  Future<bool> recordPracticeAttempt({
    ...
    if (userId != null && repository != null && cloudSaved) {
      try {
        // Always reload after an online save. The database may resolve a stale
        // client tag to the content item's canonical topic, and its aggregate
        // rebuild is the authority for the exact mastery/review state.
        await _loadRemoteState(userId, preserveLocalNewerState: true);
        _lastSyncedAt = DateTime.now();
        _errorMessage = null;
      } catch (_) {
        _errorMessage =
            'Your attempt was saved online, but the latest mastery summary could not be refreshed.';
      }
  ...
  Future<void> _loadRemoteState(
    ...
    // Ask the database to repair stale aggregate rows before reading them.
    // With the final Phase 6 migration, history is the only source of truth.
    await repository.rebuildAdaptiveMastery(userId);
    final remoteMastery = await repository.fetchTopicMastery(userId);
    final diagnostics = await repository.fetchDiagnosticAttempts(userId);
    final countedAttempts = await repository.fetchQuestionAttempts(
      userId,
      countedOnly: true,
    );
    if (_activeUserId != userId) return;

    final rebuilt = _rebuildFromHistory(diagnostics, countedAttempts);
    ...
    _diagnosticCompleted =
        diagnostics.isNotEmpty || (hasPendingDiagnostic && _diagnosticCompleted);
